...
class Orders extends Admin_Controller
{
	...
	*/
	public function fetchOrdersData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if (in_array('viewOrder', $this->permission)) {
				$buttons .= '<a style="font-size: 25px;" target="__blank" href="' . base_url('orders/invoice/' . $value['s_no'] . '/' . $value['invoice_no']) . '" class="btn btn-default"><i class="fa fa-print"></i></a>';
			}
	...
	*/
	...
	public function invoice($id, $invoice_no)
	{
		...
		if ($id) {
			$order_data = $this->model_orders->getOrdersData($id);
			$orders_items = $this->model_orders->getOrdersItemData($invoice_no);
			$footer_items = $this->model_terms->getTermsDataInOrder($id);
			$company_info = $this->model_company->getCompanyData(1);
			$party_data = $this->model_party->getPartyData($order_data['party_id']);
			$bank_details = $this->model_company->getBankDetails();
